...
stats <- txt_freq(stats$token, exclude = c("ann."))
stats$key <- factor(stats$key, levels = rev(stats$key))
barchart(key ~ freq, data = head(stats, 10), col = "cadetblue", 
         main = "Наиболее часто встечающиеся существительные", xlab = "Freq")
...
stats <- subset(x, upos %in% c("NOUN")) 
stats <- txt_freq(stats$lemma, exclude = c("ann."))
stats$key <- factor(stats$key, levels = rev(stats$key))
barchart(key ~ freq, data = head(stats, 10), col = "cadetblue", 
         main = " ", xlab = " ")
